...
int update_thread(const char *box_id, char *id, char *file_name, char *md5, char *box_folder);
...
void ParseJSON(cJSON *node, const char *box_id)
{
	...
	cJSON *fileArray = cJSON_GetObjectItem(node, "file_manage");
	char *type = NULL, *id = NULL, *file_name = NULL, *md5 = NULL, *box_folder = NULL;
	...
	while(fileList != NULL)
	{
		...
		if(strcmp(type, "update") == 0)
			update_thread(box_id, id, file_name, md5, box_folder);
...
int delete_thread(const char *box_id, char *id, char *file_name, char *md5, char *box_folder)
{
	...
	else
	{
		...
		sprintf(request_url, "http://www.ailvgobox.com/box_manage_2/file_feedback_1.php?box_id=%s&id=%s", box_id, id);
...
int update_thread(const char *box_id, char *id, char *file_name, char *md5, char *box_folder)
{
	...
	sprintf(sh_cmd, "curl -o /ailvgo/temp/%s http://www.ailvgobox.com/sysfile_manage/update/%s", file_name, file_name);
    printf("sh_cmd : %s\n", sh_cmd);
	if((stream = popen(sh_cmd, "r")) == NULL)
        	printf("popen failed\n");
	memset(result, 0, sizeof(result));
    ...
    if(access(filename_all, W_OK) == 0)
    {
        ...
        if(strcmp(md5, md5_value) == 0)
		{
            ...
            printf("delete zipfile...\n");
            memset(sh_cmd, 0, sizeof(sh_cmd));
            printf(sh_cmd, "rm -f /ailvgo/temp/%s", file_name);
            printf("sh_cmd : %s\n", sh_cmd);
            system(sh_cmd);
            sleep(2);
            printf("update complete!\n");
...
int upload_thread(const char *box_id, char *id, char *file_name, char *md5, char *box_folder)
{
	...
	else
	{
        ...
        printf("filename : %s\n", filename);
		memset(request_url, 0, sizeof(request_url));            
		sprintf(request_url, "http://www.ailvgobox.com/box_manage_2/file_query_1.php?file_name=%s&folder=%s", filename, uploadfile_folder);
        printf("request_url : %s\n", request_url);
...
int main()
{
	...
	sprintf(request_url, "http://www.ailvgobox.com/box_manage_2/file_manage_1.php?box_id=%s", box_id_tmp);
	printf("request_url : %s\n", request_url);
