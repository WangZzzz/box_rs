...
#define urllog "/ailvgo/url_log"
#define reponselog "/ailvgo/reponse_log"
...
void reponse_log(char *url ,int len)
{
	FILE *fp;
	char content[100]={0};
	fp = fopen(reponselog, "a");
	sprintf(content, "%s----%d----- \n", url,len);
	fputs(content,fp);
	fclose(fp);
}
...
int main()
{
   ...
   int cnt, j, tmp,x=0,y=0,tmp1=0;
	...
	while(1)
	{
		...
		//	printf("id:%02x%02x\n",buffer_server_rec[0],buffer_server_rec[1]);
			cache_response[i][0] = buffer_server_rec[0];
			cache_response[i][1] = buffer_server_rec[1];
			printf("%%%%%%%\n");
			for(y=0; y<cache_len[i]; y++)
			{
					printf("%02x",cache_response[i][y]);
			}
			printf("\n");
		//	sleep(1);
			tmp1 = 0;
	...
	//		buffer_client_rec[len_client_rec] = '\0';
			reponse_log(url,len_client_rec);
			strcpy(cache_url[m], url);
			cache_len[m] = len_client_rec;
	//		memset(cache_response[m],0,sizeof(cache_response[m]));
	/*		for(x=0; x<len_client_rec;x++)
			{
				cache_response[m][x] = buffer_client_rec[x];
			//	printf("%02x",cache_response[m][x]);
				printf("%02x",buffer_client_rec[x]);
			}
	*/		printf("\n");
		//	cache_response[m][len_client_rec] = '\0';
			for(x=0; x<cache_len[m]; x++)
			{
				printf("%02x",cache_response[m][x]);
			}
			printf("\n");
		//	printf("%02x---%02x--%02x--\n",&(cache_response[0][0]),&(cache_response[0][1]),&(cache_response[1][0]));
		...
		//		m = 0;
			...
			{
	    // 		if((tmp1+=sendto(sockfd_server,buffer_client_rec+tmp1,(len_client_rec-tmp1),0,(struct sockaddr *)&addr_server_send,sin_size))< 0)
		...
		if(flag == 0)
		{
			...
			while(tmp1<len_server_rec)
			{
				if((tmp1+=sendto(sockfd_server,buffer_server_rec,(len_server_rec-tmp1),0,(struct sockaddr *)&addr_server_send,sin_size))< 0)
        		{	
